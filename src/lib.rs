...
impl Plugin for Swarmer {
    ...
    fn process(
        ...
        for (sample_id, channel_samples) in buffer.iter_samples().enumerate() {
            ...
            let gain = self.params.gain.smoothed.next(); // Get smoothed dB value
            let mut gain_lin = db_to_gain(gain);
...
nih_export_clap!(Swarmer);
nih_export_vst3!(Swarmer);
